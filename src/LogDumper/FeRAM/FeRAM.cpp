...
const uint32_t FRAM_CS_PIN0 = 7;
const uint32_t FRAM_CS_PIN1 = A1;
const uint32_t FRAM_CS_PIN2 = A2;

FRAM fram0(FRAM_CS_PIN0);
FRAM fram1(FRAM_CS_PIN1);
FRAM fram2(FRAM_CS_PIN2);
...
void setup()
{
  ...
  MsgPacketizer::subscribe_manual(0x0A,
                                  [&](uint32_t millis, float loggerUsage, uint8_t framNumber)
                                  {
                                    Serial.print(millis);
                                    Serial.print(",");
                                    Serial.print(loggerUsage);
  ...
  digitalWrite(LED_BUILTIN, LOW); // 内蔵LEDを初期状態で消灯
