...
void Altimeter::initialize(uint32_t i2cAddress)
{
    ...
    _lps28->setFIFOmode(true, LPS28_FIFO_FIFO);
}
...
{
    _pressure_hPa = _lps28->getPressure(); // getPressure() は[hPa]で帰ってくる
...
{
    _referencePressure_hPa = referencePressure_kPa;
}
...
void Altimeter::setAutoReferencePressure(bool enable)
{
    ...
    _referencePressure_hPa = _lps28->getReferencePressure(); // getReferencePressure() は[Pa]で帰ってくる
}
...
{
    float altitude_m = (((pow((_referencePressure_hPa / _pressure_hPa), (1.0 / 5.257))) - 1.0) * (temperature_C + 273.15)) / 0.0065;
