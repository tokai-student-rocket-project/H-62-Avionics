...
bool FlightMode::isFlying() {
  return _currentMode == Var::FlightMode::POWERED_CLIMB
    || _currentMode == Var::FlightMode::FREE_CLIMB
    || _currentMode == Var::FlightMode::FREE_DESCENT
    || _currentMode == Var::FlightMode::DROGUE_CHUTE_DESCENT
    || _currentMode == Var::FlightMode::MAIN_CHUTE_DESCENT
    || _currentMode == Var::FlightMode::LANDED;
}


bool FlightMode::isBetween(Var::FlightMode start, Var::FlightMode end) {
  return static_cast<uint8_t>(_currentMode) >= static_cast<uint8_t>(start)
    && static_cast<uint8_t>(_currentMode) <= static_cast<uint8_t>(end);
}
