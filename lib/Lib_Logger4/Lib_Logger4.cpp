...
Logger::Logger(uint32_t csFram0, uint32_t csFram1, uint32_t csFram2, uint32_t csFram3) {
  _fram0 = new FRAM(csFram0);
  _fram1 = new FRAM(csFram1);
  _fram2 = new FRAM(csFram2);
  _fram3 = new FRAM(csFram3);
}
...
void Logger::clear() {
  ...
  _fram2->clear();
  _fram3->setWriteEnable();
  _fram3->clear();
}


uint32_t Logger::write(const uint8_t* data, uint32_t size) {
  if (_offset + size >= FRAM::LENGTH * 4) {
    return size;
  }
  else if (_offset + size >= FRAM::LENGTH * 3) {
    uint32_t writeAddress = _offset - FRAM::LENGTH * 3;
    _fram3->setWriteEnable();
    _fram3->write(writeAddress, data, size);
  }
...
uint8_t Logger::framNumber() {
  if (_offset >= FRAM::LENGTH * 4) {
    return -1;
  }
  else if (_offset >= FRAM::LENGTH * 3) {
    return 3;
  }
