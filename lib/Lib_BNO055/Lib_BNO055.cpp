...
bool BNO055::begin(OperationMode mode) {
    ...
    if (id != 0xA0) {
        ...
        // Don't return false yet, try resetting anyway
    } else {
...
uint8_t BNO055::read(uint8_t reg) {
    Wire.beginTransmission(_address);
    Wire.write(reg);
    // Use true (Stop) instead of false (Repeated Start)
